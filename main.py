...
def write_to_docx():
    file_name = filedialog.asksaveasfilename(defaultextension=".docx", filetypes=[("Word Document", "*.docx")])
...
label = ctk.CTkLabel(window, width= 6, height=50, text="Upload a file which you want to transcribe", font=(20,20))
label.pack()
...
upload_button.pack()
transcribe_button = ctk.CTkButton(frame, text="transcribe audio into text", command=transcribe_file)
transcribe_button.pack()
write_button = ctk.CTkButton(frame, text="write a transcription into docx file", command=write_to_docx)
write_button.pack()
